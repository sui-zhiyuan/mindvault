...
3. **ARM 服务器虚拟化的功能差距正在关闭，但时间差要以"十年"计**。
   x86 的 KVM 嵌套虚拟化自 **Linux 3.1（2011）** 起就可用（当年是软件模拟 VMCS；此后 Haswell 一代的
   VMCS shadowing 把它硬件加速，`【待回填】`具体代际由 03 稿确认）；
   arm64 的 KVM 嵌套直到 **Linux 6.16（2025）** 才落地，且**只支持 ARMv8.4 的 FEAT_NV2**。
   鲲鹏 920（ARMv8.2）因此**不在硬件嵌套的支持范围内**。
   → 判断：**在 ARM/鲲鹏上规划"嵌套"类场景（云上跑 Docker Desktop、CI 跑 hypervisor、VDI）时必须先确认芯片代次**，
   不能按 x86 的默认假设推进。**这十多年的时间差是鲲鹏虚拟化场景最需要显式说明的一条。**
...
本文件是写作过程中的合成草稿；**最终结论已落到 `docs/virtualization/` 的两篇正式笔记** ——
`hardware-virtualization.md`（第 1–3 问）与 `vendors-and-sandboxes.md`（第 4 问）。
原计划的六项回填结果如下：

| 原缺口 | 落地位置 | 结果 |
|---|---|---|
| `01` 软件虚拟化分档数字表（含 ASPLOS 2006 原文数字） | `hardware-virtualization.md` §三 | ✅ 已回填：内核编译 265 / 393 / 484 s；forkwait 6.0 / 36.9 / 106.4 s；pgfault 1093 / 3927 / 11242 |
| `02` 嵌套性能开销量级与功能子集 | 同上 §五 | ✅ 数量级已回填（Turtles 14.5% / −7.82% / L0 2.28%→5.17%）；**功能子集清单仍未证实** |
| `03` Intel / AMD 代际与缺失代价 | 同上 §七 | ✅ 已回填为能力矩阵；⚠ 4 项代际存在来源分歧，已在笔记中标注 |
| `04` ARM 架构版本对应 + 鲲鹏 950 | 同上 §八 | ✅ ARM 版本对应已回填；**鲲鹏 950 的架构版本与是否具备 FEAT_NV2 仍未证实** |
| `05` Broadcom 时间线 + 云厂商卸载架构 | `vendors-and-sandboxes.md` §二 / §三 | ✅ 已回填；⚠ 未取得 Broadcom SPD 原文与 Nitro 血统的官方说明 |
| `06` 沙箱六类对照数字 | 同上 §四 / §五 | ✅ 已回填；⚠ CubeSandbox 官方 README 与自家 benchmark 口径相差 4~5 倍，已并列引用 |

**仍未闭合的三项**（已写进正式笔记的未决问题）：
1. 现代硬件（Haswell 之后 / Zen / ARMv8.4-NV）上嵌套虚拟化的可靠开销百分比；
2. TDX 与 SEV-SNP 相对裸机的启动与吞吐损耗；
3. 鲲鹏 950 的 ARM 架构版本，以及其"四层隔离"是否基于 RME。
